...
| Fly.io Runtime OCR client + Mistral vision fallback | **SUPERSEDED (2026-07-22, Wave 13)** | ocr_service_simple.go callFlyOCR / ocrWithMistralVision (as of this snapshot) | OSS retired Fly.io entirely: `ocrWithMistral` submits native PDF/image to Mistral OCR 4 (`pkg/ocr/mistralocr`, schema-shaped Document AI extraction) with an offline tesseract fallback via the local ACEEngine (`ocrWithLocalEngine`); `ProcessWithFlorence2/Tesseract/GPU` bindings removed | Deliberate divergence — see FABLE_WAVE13_SPEC_PERCEPTION_AND_PRINT.md / FABLE_WAVE13_REPORT.md. This row is now stale for OSS; left for historical PH-side reference only. |
