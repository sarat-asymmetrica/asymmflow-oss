...
//     returned a bare customer word like "NPC" as the folder number for inputs
//     such as "NPC LIT" — a real folder number always contains a digit.
...
//     good "EH-103-26" with "NPC".
//
// canonicalOpportunityKey then keyed on the corrupted folder, so distinct
// opportunities collapsed (two Npc deals both keyed "NPC", etc.).
//
...
func TestOneDriveOpportunitiesDoNotCollapseToCustomerWord(t *testing.T) {
	...
	npcB := Opportunity{
		Source:       "2026_onedrive",
		FolderName:   "OTH-04-26 NPC PARTS",
	...
	keyA := canonicalOpportunityKey(normalizeOpportunityForList(npcA))
	keyB := canonicalOpportunityKey(normalizeOpportunityForList(npcB))
	keyV := canonicalOpportunityKey(normalizeOpportunityForList(veolia))

	// The actual collapse: the two Npc deals must NOT share a canonical key
	// (under the old behavior both became "NPC").
	require.NotEqualf(t, keyA, keyB,
		"two distinct Npc opportunities collapsed to the same canonical key %q", keyA)
	require.NotEqual(t, keyA, keyV, "Npc and Veolia opportunities collapsed to the same key")
	require.NotEqual(t, keyB, keyV, "Npc and Veolia opportunities collapsed to the same key")

	// And no canonical key may be a bare customer word.
	for _, k := range []string{keyA, keyB, keyV} {
		require.NotEqual(t, "NPC", k, "canonical key must not be a bare customer word")
