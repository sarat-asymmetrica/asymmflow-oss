...
| 8 | 23 write-action bindings | **WIRED — SPLIT (owner ruling G1.1)** | All routing/guards in `resolveCreate/Update/ApprovalAction`. The **19 draft/update-class** bindings are WIRED for real: the resolvers assemble each payload (builders ported verbatim from the legacy ButlerScreen) and return an `execute` thunk calling a typed bridge wrapper; the confirming HUMAN is the acting actor (Go handlers stamp `getCurrentUserID`, never "butler"; none of the 19 takes an actor arg). The **4 approve-class** bindings (`ApprovePurchaseOrder`, `ApproveStockAdjustment`, `ApproveSupplierInvoice`, `ApproveCostingSheet`) are **RETIRED** — never imported into the bridge, redirected to the Approvals Queue at the resolver, with a defense-in-depth guard in `executeButlerAction`. Butler's costing-sheet "update status" also redirects (the Go `UpdateCostingSheet` Omits `Status`, so wiring it would be a pretend-persist). Mechanically proven by `tests/butler-authority-boundary.test.ts` (15 cases): the 4 approve names can never resolve to an executable action. |
