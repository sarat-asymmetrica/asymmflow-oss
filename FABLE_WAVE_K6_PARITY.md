...
| Goods Received · `grns` | GRNScreen | Ledger | ✅ | real | **wired** (partial) | ✅ `UpdateGRNQCStatus`(QC verdict form)+`CompleteGRN`(confirm) 🔥 (R5; qcBy from session); ⚠️ Receive-from-PO (`ReceiveAgainstPO`, per-line receive qty) stays SLOT — needs LineItemsEditor engine | ☐ |
...
| Business Settings · `business-settings` | SettingsScreen (split) | Bespoke | ✅ | **real** (keys fixed R3) | mock (INTEG) | `UpdateSettings` DELIBERATELY GAPPED (R3 confirmed: full-overwrite would wipe `apiKeys`/`folders`; needs a merge-safe path) — but the fetch-side `mapSettings` was FIXED to the real keys (`companyName`/`currency`/`business.*`), a latent real-mode blank-read bug | ☐ |
