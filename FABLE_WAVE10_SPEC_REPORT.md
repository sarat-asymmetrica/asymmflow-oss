...
**[TASTE]** press scale = **0.97** (Constitution IV.2), the more pronounced of the two candidates vs the reference impl's 0.985 — owner: check it doesn't feel twitchy on rapid double-clicks. **[FLAG]** the login "Back" link and a "+ Add" pill gain visible padding/border/hover once on `<Button variant="ghost">` — an intentional normalization I authorized, but a visible change.

### B5 — Rituals, operator language, brand slot ✅ (committed 4ad3d3a)
**(a) Six-document closing set** — `DealDocumentChecklist.svelte` mounts below the timeline on OrderDetail: offer / order confirmation / delivery note / invoice / statement entry / receipt, each present-or-missing with a link where a route exists, derived READ-ONLY from the deal assembly (new `Documents` field on `DealTimeline`, separate from `Nodes` so the B3 stepper is untouched; zero writes, `finance:view`-gated). "Statement entry" has no first-class entity — rendered honestly from the "invoice has left Draft" signal the app's own ledger code uses, never invented. Full-set + PAID shows a quiet complete state via `--motion-settle` (no sound/extra animation — B4's sound already lives on the posting click).
**(b) Empty states** — ~12 generic list-view messages rewritten in operator/domain-noun voice ("No RFQs yet — enquiries you log land here.", etc.) + `DataTable`'s default ("Nothing here yet."). In-place, behavior frozen; no illustrations/mascots.
**(c) Brand slot** — `frontend/src/lib/brand.ts` is the ONE synthetic identity source (`AsymmFlow`/`AF`/`var(--brand-indigo)`), consumed by the sidebar header and login/lock — **the stale hardcoded "PH Trading"/"PH Holdings" is gone** (also closes a synthetic-identity leak). Deployment re-skin = a **gitignored `brand.local.ts`** shallow-merged via `import.meta.glob` (proven end-to-end with a throwaway green/ProofCo override, then removed — committed tree is 100% synthetic). PDF headers already config-driven via `overlay.json`; desktop icon is a build-asset swap. `docs/DEPLOYMENT_BRANDING.md` documents every slot + the two-step recipe (one config/token override + one icon build-asset swap, zero source edits).
**AC:** closed deal's document set verifiable at a glance; empty states read like a colleague wrote them; rebrand = one override + one build-asset, proven live, no non-synthetic identity committed.
**[FLAG]** the accent-green vs semantic-success check: distinguishable, and the timeline is monochrome so no "done = green" collision — but sample the flagship's exact green beside a success badge at gate. Purchase-Orders empty-state verb ("raise a PO") — confirm house verb.
**[MINOR]** the checklist re-fetches `GetDealTimeline` (same call the timeline makes) → two cheap read round-trips on OrderDetail where one could serve both; correctness fine, optional future collapse.
...
**B1 — press scale [TASTE].** 0.97 (Constitution IV.2), chosen over the reference impl's subtler 0.985 for a more tactile press. Focus ring converged on `--focus-ring-color` (resolves to the existing near-black — no visual change today, one source now). Skeletons chosen over spinners for the 5 hot spots (content-shaped, zero shift). **Owner look:** the press depth on rapid clicks; the login "Back"/"+ Add" controls now look like real buttons (padding/border) rather than bare links.
